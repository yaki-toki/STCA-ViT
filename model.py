...
Architecture overview:
    ...
    x (B, 3, T, H, W)
    ├── VideoMAE ViT-Base Backbone  → b_back  (B, 768)
...
class OrthogonalLoss(nn.Module):
    ...
    def forward(self, b_back, f_stca, f_mgu, f_person=None):
        cos_loss = (
            self._cosine_loss(b_back, f_stca) +
            self._cosine_loss(b_back, f_mgu) +
            self._cosine_loss(f_stca, f_mgu)
        )
        decorr_loss = (
            self._decorrelation_loss(b_back, f_stca) +
            self._decorrelation_loss(b_back, f_mgu) +
            self._decorrelation_loss(f_stca, f_mgu)
        )
        if f_person is not None:
            cos_loss = cos_loss + (
                self._cosine_loss(b_back, f_person) +
                self._cosine_loss(f_stca, f_person) +
                self._cosine_loss(f_mgu, f_person)
            )
            decorr_loss = decorr_loss + (
                self._decorrelation_loss(b_back, f_person) +
                self._decorrelation_loss(f_stca, f_person) +
                self._decorrelation_loss(f_mgu, f_person)
            )
...
class WTALLoss(nn.Module):
    ...
    4. Smoothness: minimizes action_score variation between adjacent frames
    ...
    def forward(self, tal_outputs, labels, skip_mil=False):
        ...
        Args:
            tal_outputs: dict from TemporalActionLocalizationHead
                ...
                - mil_logits: (B, num_classes) MIL-aggregated clip-level prediction
                - pseudo_targets: (B, T) MGU w_temp based pseudo-labels
            labels: (B,) clip-level class labels
            skip_mil: skip MIL CE loss (e.g. during MixUp/CutMix)
        Returns:
            total_loss: scalar
        ...
        # 3. Sparsity Loss (mean of action_scores approaches target_sparsity)
        ...
        # 4. Smoothness Loss (minimize action_score differences between adjacent frames)
        ...
        # 5. Background Entropy Loss (uniform class distribution over background frames)
...
class VideoMAEBackbone(nn.Module):
    ...
    - Input: (B, C=3, T=16, H, W) — arbitrary resolution (internally resized to 224x224)
    - Output: b_back (B, 768)

    Note: VideoMAE requires 224x224 input internally. Inputs at other resolutions are
    automatically resized via bilinear interpolation before processing.
    Other modules (STCA, MGU, PersonAttention) operate on the original resolution,
    so smaller inputs such as 112x112 can provide a speed advantage.
    """
    def __init__(self, pretrained_name="MCG-NJU/videomae-base-finetuned-kinetics",
        ...
        # Gradient checkpointing (reduces memory, allows larger batch sizes)
        ...
        if freeze_layers > 0:
            # Freeze embeddings (requires_grad handled in forward when gradient checkpointing is used)
    ...
    def forward(self, x):
        """
        Args:
            x: (B, C, T, H, W) - input video (arbitrary resolution, auto-resized to 224x224 internally)
        Returns:
            b_back: (B, 768) - avg pool over patch tokens
        """
        B, C, T, H, W = x.shape

        # VideoMAE requires 224x224 input; resize automatically if needed
        if H != 224 or W != 224:
        ...
        # When using gradient checkpointing with frozen layers, ensure requires_grad=True
        # on the input to avoid checkpoint warnings when the frozen embedding has no grad
...
class STCAModule(nn.Module):
    """
    STCA Module (Spatio-Temporal Cross-Attention).

    Extracts spatial and temporal tokens independently from raw video x,
...
class MGUModule(nn.Module):
    """
    Motion Gated Unit - Optical Flow style motion feature extractor.

    Extracts motion features independently from raw video x using
    Sobel gradients and bidirectional temporal differences.
    """
    ...
    def forward(self, x, external_feat=None, return_temporal=False):
        ...
        Returns:
            return_temporal=False: out (B, embed_dim)
            return_temporal=True:  (out, temporal_info_dict)
        """
        B, C, T, H, W = x.shape
        ...
        # Preserve M_enc before boundary application for WTAL head
        B_mask = self.boundary_conv(M_enc)
        if return_temporal:
            M_enc_raw = M_enc  # feature before boundary application
        ...
        if return_temporal:
            # Detach: WTAL is an auxiliary loss; gradients should not propagate back through MGU.
            # This releases the backward graph of large 5D tensors to avoid OOM.
            temporal_info = {
                'M_enc': M_enc_raw.detach(),      # (B, D, T', H', W') — before boundary application
                'B_mask': B_mask.detach(),         # (B, 1, T', H', W')
                'w_temp': w_temp.detach(),         # (B, 1, T', 1, 1)
            }
...
class PersonAttentionModule(nn.Module):
    """
    YOLO-based person-focused feature extractor.
    """
    def __init__(self, in_channels=3, embed_dim=768, dropout=0.3,
...
class TemporalActionLocalizationHead(nn.Module):
    ...
    Flow:
        M_enc (B, D, T, H, W)
            ...
            `-- MIL aggregation: T-CAM x attention -> (B, num_classes) [mil_logits]
    """
    ...
    def forward(self, M_enc, B_mask, w_temp):
        """
        Args:
            M_enc: (B, D, T, H, W) — MGU motion feature (before boundary application)
...
class ResidualContributionFusion(nn.Module):
    """
    Residual Contribution Fusion.

    If module_embed_dim differs from embed_dim, a linear projection aligns dimensions.
...
class MultiHeadEnsembleClassifier(nn.Module):
    def __init__(self, backbone_dim=768, embed_dim=768, module_embed_dim=None, num_classes=400, dropout=0.3):
        ...
        # Module heads: take module_embed_dim input (detached raw module features)
        ...
        # Fused head: takes embed_dim input (fusion output)
...
class STCAViT(nn.Module):
    """
    STCA-ViT: Spatio-Temporal Cross-Attention Vision Transformer.
    ...
    Setting module_embed_dim smaller than embed_dim significantly reduces memory usage of
    the auxiliary modules. Fusion and Classifier project module_embed_dim -> embed_dim.

    When use_wtal=True, MGU temporal features are reused to output per-frame action
    probabilities. Pass compute_wtal=True in forward() to include tal_outputs in the result.
    """
    def __init__(self, num_classes=400, embed_dim=768, module_embed_dim=None, dropout=0.3, drop_path=0.2,
        ...
        # If module_embed_dim is not specified, default to embed_dim
        if module_embed_dim is None:
            module_embed_dim = embed_dim
        ...
        # ==================
        # 2. STCA Module (Spatio-Temporal Cross-Attention, raw video x -> module_embed_dim)
        # ==================
        ...
        # ==================
        # 3. MGU Module (raw video x -> module_embed_dim motion features)
        # ==================
        ...
        # ==================
        # 4. Person Attention Module (-> module_embed_dim)
        # ==================
        self.use_person_attention = YOLO_AVAILABLE
        if self.use_person_attention:
    ...
    def forward(self, x, return_all=False, compute_ortho_loss=False, compute_wtal=False):
        ...
        # 2. STCA Module (raw video x)
        f_stca = self.stca_module(x)  # (B, module_embed_dim)

        # 3. MGU (raw video x) — also return temporal features when WTAL is active
        ...
        # 4. Person Attention (raw video x)
        if self.use_person_attention:
            f_person = self.person_module(x)  # (B, module_embed_dim)
        else:
            f_person = torch.zeros_like(f_mgu)
        ...
        # 7. Orthogonal Loss (project all features to the same dimension before comparison)
        ortho_loss = None
        if compute_ortho_loss:
            b_proj = self.fusion.backbone_proj(b_back)
            stca_proj_feat = self.fusion.stca_proj(f_stca)
            mgu_proj_feat = self.fusion.mgu_proj(f_mgu)
            person_proj_feat = self.fusion.person_proj(f_person) if self.use_person_attention else None
...
def stca_vit(num_classes=400, **kwargs):
    """
    STCA-ViT with VideoMAE ViT-Base backbone (embed_dim=768, num_heads=12).

    module_embed_dim: dimension of auxiliary modules (STCA, MGU, PersonAttention).
                      Defaults to None, which is equivalent to embed_dim (768).
                      Setting this to 256 reduces auxiliary module memory by ~75%.
    use_wtal: if True, enables the WTAL Temporal Action Localization head.
              Defaults to False, preserving identical behavior to the base model.
    """
...
stca_vit_v2_5_parallel = stca_vit
