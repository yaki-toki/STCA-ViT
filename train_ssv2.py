"""
STCA-ViT Training for Something-Something-v2 (SSv2)
====================================================

Written for SSv2 dataset training.

Dataset structure:
- /home/user/data/something-something-v2/20bn-something-something-v2/{id}.webm
- /home/user/data/something-something-v2/labels.json     {"class_name": str_idx, ...}
- /home/user/data/something-something-v2/train.json      [{id, label, template, placeholders}, ...]
- /home/user/data/something-something-v2/validation.json [{id, label, template, placeholders}, ...]
- /home/user/data/something-something-v2/test.json       [{id}, ...]

Label mapping:
- Remove square brackets ([something]) from the template field in train/val JSON
- Look up the cleaned text in labels.json -> integer class index

Usage (Single GPU):
---------
python train_ssv2.py --try_number 1

Usage (WTAL enabled):
---------
python train_ssv2.py --try_number 1 --use_wtal

Usage (Accelerate - Multi-GPU):
---------
accelerate launch train_ssv2.py --try_number 1
"""
...
class VideoRandAugment:
    ...
    def _autocontrast(self, img):
        ...
        for c in range(3):
            ...
            if hi > lo:
                result[:, :, c] = np.clip(
                    (ch.astype(np.float32) - lo) * 255.0 / (hi - lo), 0, 255
                ).astype(np.uint8)
            else:
                result[:, :, c] = ch
        return result
    ...
    def _adjust_color(self, img, factor):
        ...
        return np.clip(
            img.astype(np.float32) * factor + gray_3ch.astype(np.float32) * (1 - factor),
            0, 255
        ).astype(np.uint8)
    ...
    def _adjust_sharpness(self, img, factor):
        ...
        return np.clip(
            img.astype(np.float32) * (1 - factor) + sharpened.astype(np.float32) * factor,
            0, 255
        ).astype(np.uint8)
...
class VideoRandomErasing:
    """Random Erasing on video tensors - maintains temporal consistency"""
...
class SSv2Dataset(Dataset):
    ...
    Video: {data_root}/20bn-something-something-v2/{id}.webm
    Labels: {data_root}/labels.json  ->  {"class_name": str_idx}
    Splits:
      train   -> {data_root}/train.json      [{id, label, template, placeholders}, ...]
      val     -> {data_root}/validation.json  [{id, label, template, placeholders}, ...]
      test    -> {data_root}/test.json        [{id}, ...]  (no labels)

    Label mapping:
      Remove square brackets like [something] from the template field
      -> look up key in labels.json -> integer index
    """
    ...
    # SSv2 official statistics (for reference)
    SSV2_MEAN = np.array([0.485, 0.456, 0.406], dtype=np.float32)
    SSV2_STD  = np.array([0.229, 0.224, 0.225], dtype=np.float32)

    def __init__(self, data_root, split='train', clip_length=16, frame_stride=2,
                 w_size=224, h_size=224, random_flip=True,
        ...
        self.video_dir = os.path.join(data_root, '20bn-something-something-v2')
        self.split = split
        ...
        # Load labels.json: {"class_name": "str_idx", ...}
        labels_path = os.path.join(data_root, 'labels.json')
        with open(labels_path, 'r', encoding='utf-8') as f:
            raw_labels = json.load(f)

        # str_idx -> int, sort class list
        # raw_labels values may be strings or integers
        self.class_to_idx = {k: int(v) for k, v in raw_labels.items()}
        num_classes = max(self.class_to_idx.values()) + 1
        self.idx_to_class = {v: k for k, v in self.class_to_idx.items()}
        self.classes = [self.idx_to_class[i] for i in range(num_classes)]
        self.num_classes = num_classes

        print(f"SSv2 number of classes: {num_classes}")

        # Load samples
        self.samples = self._load_samples()
        split_name = {'train': 'Train', 'val': 'Validation', 'test': 'Test'}[split]
        print(f"SSv2 {split_name}: {len(self.samples)} videos")
        print(f"   Frame sampling: {clip_length} frames x stride {frame_stride} = {clip_length * frame_stride} frame span")

    @staticmethod
    def _template_to_class_name(template: str) -> str:
    ...
    def _load_samples(self):
        ...
        json_path = os.path.join(self.data_root, split_file_map[self.split])
        ...
        for item in annotations:
            ...
            if self.split == 'test':
                ...
                class_name = self._template_to_class_name(template)
        ...
        if missing_label > 0:
            print(f"  Warning: {missing_label} samples excluded due to missing labels")
    ...
    def _load_video(self, video_path):
        ...
        try:
            ...
            frames_tensor = vr.get_batch(frame_indices_clamped)  # (T, H, W, C)
            ...
            with accelerator.autocast():
                if use_aux_loss:
                    ...
                    if wtal_active and 'tal_outputs' in outputs_dict:
                        ...
                        wtal_loss, _ = unwrapped.wtal_loss_fn(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='SSv2 Training with STCA-ViT (VideoMAE)')

    # Data
    parser.add_argument('--data_root', type=str,
                        default='/home/user/data/something-something-v2',
                        help='SSv2 data root directory (contains labels.json, train.json, etc.)')
    parser.add_argument('--clip_length', type=int, default=16)
    parser.add_argument('--frame_stride', type=int, default=2,
                        help='Frame sampling stride (SSv2 is ~12fps, so 1-2 is recommended)')
    ...
    if args.resume:
        ...
        if accelerator.is_main_process:
            print("\n" + "=" * 80)
            print("Checkpoint pre-validation")
            ...
            if isinstance(checkpoint_preview, dict):
                ...
                ckpt_loss = checkpoint_preview.get('val_loss', checkpoint_preview.get('best_val_loss', '?'))
                ckpt_acc = checkpoint_preview.get('val_acc', checkpoint_preview.get('best_val_acc', '?'))
                ...
                print(f"   Checkpoint format is invalid.")
                del checkpoint_preview
                exit(1)
    ...
    # Model name / WandB
    # ========================
    model_name = f"{args.try_number}_STCA_ViT_VideoMAE_SSv2_{args.clip_length}f_{image_size}"

    if accelerator.is_main_process:
        print(f"\n{'='*80}")
        print(f"Something-Something-v2 - STCA-ViT (VideoMAE ViT-Base)")
        ...
        print(f"   Frame Stride: {args.frame_stride} (SSv2 ~12fps)")
        print(f"   Accelerator Device: {accelerator.device}")
        print(f"{'='*80}")

        wandb.init(
            project="SSv2_STCA_ViT_VideoMAE",
            name=model_name,
            config={
                ...
                "use_temporal_aug": args.use_temporal_aug,
    ...
    with accelerator.main_process_first():
        train_dataset = SSv2Dataset(
            ...
            w_size=image_size, h_size=image_size, random_flip=True,
        ...
        val_dataset = SSv2Dataset(
            ...
            clip_length=args.clip_length, frame_stride=args.frame_stride,
            w_size=image_size, h_size=image_size, random_flip=False,
        )
        test_dataset = SSv2Dataset(
    ...
    # Test data has no labels; use a separate inference script if needed
    ...
    if accelerator.is_main_process:
        ...
        print(f"\nEnabled features:")
        print(f"   - Gradient Reversal: {'ON' if args.use_grl else 'OFF'}")
        print(f"   - Orthogonal Loss:   {'ON' if args.use_ortho_loss else 'OFF'}")
        print(f"   - Multi-Head:        {'ON' if args.use_multi_head else 'OFF'}")
    ...
    # ========================
    backbone_lr  = args.learning_rate * args.backbone_lr_ratio
    module_lr    = args.learning_rate * 1.0
    fusion_lr    = args.learning_rate * 0.8
    head_lr      = args.learning_rate * 1.5
    wtal_lr      = args.learning_rate * 2.0
    ...
    if accelerator.is_main_process:
        print(f"\nLayer-wise LR:")
        print(f"   - VideoMAE Backbone:        {backbone_lr:.2e}")
        print(f"   - Cross-Attention/MGU/Person:{module_lr:.2e}")
        print(f"   - Fusion:                   {fusion_lr:.2e}")
        print(f"   - Classification Heads:     {head_lr:.2e}")
        if args.use_wtal:
            print(f"   - WTAL Head:                {wtal_lr:.2e}")

    param_groups = [
        {'params': backbone_params,   'lr': backbone_lr, 'weight_decay': args.weight_decay},
        {'params': cross_attn_params, 'lr': module_lr,   'weight_decay': args.weight_decay},
        {'params': mgu_params,        'lr': module_lr,   'weight_decay': args.weight_decay},
        {'params': fusion_params,     'lr': fusion_lr,   'weight_decay': args.weight_decay},
        {'params': classifier_params, 'lr': head_lr,     'weight_decay': args.weight_decay * 2.0},
    ]
    if person_params:
        param_groups.append({'params': person_params, 'lr': module_lr, 'weight_decay': args.weight_decay})
    if wtal_params:
        param_groups.append({'params': wtal_params, 'lr': wtal_lr, 'weight_decay': args.weight_decay})
    ...
    if args.use_class_weights:
        ...
        class_weights = {cls: np.sqrt(sum(class_counts.values()) / count)
    ...
    patience_counter = 0
    start_epoch = 0
    best_model_path    = f"{args.try_number}_STCA_ViT_VideoMAE_SSv2_{args.clip_length}f_{image_size}_Best.pth"
    trained_model_path = f"{args.try_number}_STCA_ViT_VideoMAE_SSv2_{args.clip_length}f_{image_size}_Trainover.pth"
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if accelerator.is_main_process:
            ...
            if hasattr(unwrapped_model, 'get_contribution_weights'):
                ...
                if isinstance(contrib_weights, dict):
                    print(f"Contribution: alpha={contrib_weights.get('alpha (stca)', 0):.3f}, "
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            if accelerator.is_main_process:
                unwrapped_ema = accelerator.unwrap_model(model_ema.module)
                unwrapped_train = accelerator.unwrap_model(model)
                torch.save({
                    'epoch': epoch + 1,
                    'model_state_dict':       unwrapped_ema.state_dict(),
                    'train_model_state_dict': unwrapped_train.state_dict(),
    ...
    if accelerator.is_main_process:
        print(f"\n{'='*80}")
        print(f"Final Validation Set Evaluation (Best Model)")
        print(f"{'='*80}\n")

        checkpoint = torch.load(best_model_path, map_location=device, weights_only=False)
        model_ema.module.load_state_dict(checkpoint['model_state_dict'])

        final_val_loss, final_val_acc, final_val_f1, _ = validate_epoch(
            model_ema.module, val_loader, val_criterion, accelerator, use_tta=args.use_tta
        )
