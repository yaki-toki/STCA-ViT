"""
STCA-ViT Training for UCF-101
====================================================

Based on train_hmdb51.py, adapted for the UCF-101 dataset.

Dataset structure:
- ../ucf101/ucf101/{ClassName}/v_{ClassName}_g{01-25}_c{01-06}.avi
- group 01~19 -> Training + Validation (8:2 stratified split)
- group 20~25 -> Test (approximates standard split1)

Usage (Single GPU):
---------
python train_ucf101.py --try_number 1

Usage (with WTAL enabled):
---------
python train_ucf101.py --try_number 1 --use_wtal

Usage (Accelerate - Multi-GPU):
---------
accelerate launch train_ucf101.py --try_number 1 --use_wtal
"""
...
class UCF101Dataset(Dataset):
    ...
    - group 01~{train_group_max} -> Training + Validation (stratified split)
    - group {train_group_max+1}~25 -> Test

    Default split: group 1~19=train/val, group 20~25=test (approx. 76:24)
    """
    ...
    def __init__(self, data_root, split='train', clip_length=16, frame_stride=2,
                 w_size=224, h_size=224, val_ratio=0.2, seed=42,
                 train_group_max=19, random_flip=True,
        ...
        # Class list (folder name = class name, only folders containing .avi files)
        ...
        print(f"UCF-101 number of classes: {len(self.classes)}")

        # Load samples
        self.samples = self._load_samples()
        split_name = {'train': 'Train', 'val': 'Validation', 'test': 'Test'}[split]
        print(f"UCF-101 {split_name}: {len(self.samples)} videos")
        print(f"   Frame sampling: {clip_length} frames x stride {frame_stride} = {clip_length * frame_stride} frame span")

    def _load_samples(self):
        """Split train/val/test based on group number."""
        train_samples = []
        test_samples = []

        # Extract group number from v_{ClassName}_g{group}_c{clip}.avi
        ...
        # Stratified split of train samples into train/val per class
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='UCF-101 Training with STCA-ViT (VideoMAE)')

    # Data
    parser.add_argument('--data_root', type=str, default='../ucf101/ucf101',
                        help='UCF-101 video directory (contains per-class subdirectories)')
    parser.add_argument('--clip_length', type=int, default=16)
    parser.add_argument('--frame_stride', type=int, default=4,
                        help='Frame sampling stride (stride=4 recommended for UCF-101 at 30fps)')
    parser.add_argument('--train_group_max', type=int, default=19,
                        help='Max group number for train/val (groups 1~train_group_max -> train/val, rest -> test)')
    ...
    # Model name / WandB
    # ========================
    model_name = f"{args.try_number}_STCA_ViT_VideoMAE_UCF101_{args.clip_length}f_{image_size}"

    if accelerator.is_main_process:
        print(f"\n{'='*80}")
        print(f"UCF-101 - STCA-ViT (VideoMAE ViT-Base)")
        ...
        print(f"   Data Split: group 1~{args.train_group_max} (Train+Val), group {args.train_group_max+1}~25 (Test)")
        print(f"   Accelerator Device: {accelerator.device}")
        print(f"{'='*80}")

        wandb.init(
            project="UCF101_STCA_ViT_VideoMAE",
            name=model_name,
            config={
    ...
    with accelerator.main_process_first():
        train_dataset = UCF101Dataset(
            ...
            train_group_max=args.train_group_max, random_flip=True,
        ...
        val_dataset = UCF101Dataset(
            ...
            w_size=image_size, h_size=image_size,
            val_ratio=args.val_ratio, seed=args.seed,
            train_group_max=args.train_group_max, random_flip=False,
        )
    ...
    patience_counter = 0
    start_epoch = 0
    best_model_path    = f"{args.try_number}_STCA_ViT_UCF101_{args.clip_length}f_{image_size}_Best.pth"
    trained_model_path = f"{args.try_number}_STCA_ViT_UCF101_{args.clip_length}f_{image_size}_Trainover.pth"
