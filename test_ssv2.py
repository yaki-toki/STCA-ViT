"""
STCA-ViT Test Script for Something-Something-v2 (SSv2)
====================================================

Loads a trained model checkpoint and performs comprehensive evaluation
on the SSv2 validation set.
(SSv2 test.json does not contain labels, so the validation set is used for evaluation.)

Generated visualizations:
---------
1. 06_ssv2_confusion.png           - Normalized Confusion Matrix (Reds, %)
2. 07_ssv2_f1score.png             - F1-Score per Class (color-banded bar chart)
3. 08_ssv2_class_performance_detailed.png - Top/Bottom 10, distribution, Scatter
4. 09_ssv2_distribution_of_samples_per_class.png - Histogram of samples per class
5. 10_ssv2_prediction_confidence_analysis.png - Confidence analysis (including ECE)

Usage:
---------
python test_ssv2.py --checkpoint STCA_ViT_VideoMAE_SSv2_16f_224_Best.pth
python test_ssv2.py --checkpoint STCA_ViT_VideoMAE_SSv2_16f_224_Best.pth --detailed --analyze_samples 10 --save_predictions
"""
...
# ===========================
class SSv2ValDataset(Dataset):
    """
    Something-Something-v2 Validation Dataset

    Videos: {data_root}/20bn-something-something-v2/{id}.webm
    Labels: {data_root}/labels.json  ->  {"class_name": str_idx}
    Eval set: {data_root}/validation.json  ->  [{id, label, template, placeholders}, ...]

    Label mapping:
      Remove square brackets in [something] format from the template field
      -> look up labels.json key -> integer index

    Note: SSv2 test.json has no labels, so the validation set is used for final evaluation.
    """
    ...
    def __init__(self, data_root, clip_length=16, frame_stride=2,
                 w_size=224, h_size=224, multi_crop=False):
        ...
        print(f"SSv2 Validation: {len(self.samples)} videos")
    ...
    def _load_val_samples(self):
        json_path = os.path.join(self.data_root, 'validation.json')
...
# Evaluation Function
# ===========================
def evaluate(model, val_loader, device, use_tta=True, multi_crop=False, detailed=False):
    model.eval()
    all_preds = []
    all_labels = []
    all_probs = []
...
def print_per_class_accuracy(all_labels, all_preds, idx_to_class):
    ...
    print(f"{'='*70}")
    print(f"{'No.':<5} {'Class':<40} {'Correct/Total':<15} {'Accuracy':>10}")
    print(f"{'-'*70}")
    ...
    for i in range(num_classes):
        ...
        # SSv2 class names can be long; truncate for display
        display_name = class_name[:38] + '..' if len(class_name) > 40 else class_name
        print(f"{i+1:<5} {display_name:<40} {correct:>4}/{total:<8} {acc:>8.2f}%")

    print(f"{'-'*70}")
    print(f"{'Mean':<45} {'':<15} {np.mean(accuracies):>8.2f}%")
    print(f"{'='*70}")
    return accuracies
...
# ===========================
def main():
    parser = argparse.ArgumentParser(description='STCA-ViT Test for Something-Something-v2')

    parser.add_argument('--checkpoint', type=str, required=True)
    parser.add_argument('--data_root', type=str, default='/home/user/data/something-something-v2')
    ...
    parser.add_argument('--frame_stride', type=int, default=2,
                        help='Frame sampling stride (2 recommended for SSv2 ~12fps)')
    ...
    # Output
    parser.add_argument('--output_dir', type=str, default='test_results_ssv2')
    ...
    # ========================
    print(f"\n{'='*60}")
    print(f"{'TEST RESULTS (SSv2 Validation)':^60}")
    print(f"{'='*60}")
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write(f"STCA-ViT Test Results - Something-Something-v2\n")
        f.write(f"{'='*60}\n")
        f.write(f"Checkpoint: {args.checkpoint}\n")
        f.write(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Evaluation Mode: {mode}\n")
        f.write(f"Eval Split: Validation (test.json has no labels)\n")
        f.write(f"{'='*60}\n\n")
        ...
        f.write(f"Total Val Samples:   {len(results['all_labels'])}\n\n")
        f.write(f"Per-Class Accuracy:\n")
        for i in range(num_classes):
            f.write(f"   {idx_to_class[i]:<50} {class_accuracies[i]:.2f}%\n")
