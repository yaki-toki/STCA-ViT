"""
STCA-ViT Test Script for Kinetics-400
====================================================

Loads a trained model checkpoint and performs comprehensive evaluation
on the Kinetics-400 test set.

Generated visualizations:
---------
1. 06_kinetics_confusion.png               - Normalized Confusion Matrix (Reds, %)
2. 07_kinetics_f1score.png                 - F1-Score per Class (color-banded bar chart)
3. 08_kinetics_class_performance_detailed.png - Top/Bottom 10, distribution, Scatter
4. 09_kinetics_distribution_of_samples_per_class.png - Histogram of samples per class
5. 10_kinetics_prediction_confidence_analysis.png - Confidence analysis (including ECE)

Usage:
---------
python test_kinetics400.py --checkpoint STCA_ViT_VideoMAE_16frame_224_Best.pth
python test_kinetics400.py --checkpoint STCA_ViT_VideoMAE_16frame_224_Best.pth --use_wtal --detailed
"""
...
# ===========================
# Kinetics-400 Test Dataset
# ===========================
class Kinetics400TestDataset(Dataset):
    """
    Kinetics-400 Test Dataset
    - Loads test set based on test.csv annotations
    - Same preprocessing as training code (center crop, Kinetics normalization)
    - Averages num_clips clips (multi-clip evaluation)
    """
    ...
    def __init__(self, video_paths, labels, idx_to_class,
                 clip_length=16, temporal_stride=4,
                 w_size=224, h_size=224, num_clips=3):
        ...
        print(f"Kinetics-400 Test: {len(self.video_paths)} videos, "
              f"{len(idx_to_class)} classes, {num_clips} clips/video")
    ...
    def __getitem__(self, idx):
        ...
        clips = self._load_video_multi_clip(video_path)
        return clips, label, video_path

    def _get_clip_indices(self, total_frames, clip_offset=0, num_offsets=1):
        """Extract clip indices from total frames using uniform partitioning."""
        span = self.clip_length * self.temporal_stride
        if total_frames >= span:
            if num_offsets <= 1:
                start = (total_frames - span) // 2
            else:
                # Uniform partitioning: divide 0~(total_frames-span) into num_offsets segments
                step = max(0, (total_frames - span) // (num_offsets - 1))
                start = min(clip_offset * step, total_frames - span)
            indices = [min(start + i * self.temporal_stride, total_frames - 1)
                       for i in range(self.clip_length)]
        else:
            # Short video: repeat padding
            effective_stride = max(1, total_frames // self.clip_length)
            indices = [min(i * effective_stride, total_frames - 1)
                       for i in range(self.clip_length)]
        return indices

    def _load_frames(self, video_path, indices):
        try:
            ...
            return frames_tensor.numpy().astype(np.uint8), total_frames
    ...
    def _process_frames(self, frames_np):
        ...
        for t in range(frames_np.shape[0]):
            ...
            if h_orig == 0 or w_orig == 0:
                frame = np.zeros((self.h_size, self.w_size, 3), dtype=np.float32)
            else:
                # Center crop: resize shorter side to target, then center crop
                scale = max(self.h_size / h_orig, self.w_size / w_orig)
                new_h = max(int(h_orig * scale), self.h_size)
                new_w = max(int(w_orig * scale), self.w_size)
                frame = cv2.resize(frame, (new_w, new_h))
                start_y = (new_h - self.h_size) // 2
                start_x = (new_w - self.w_size) // 2
                frame = frame[start_y:start_y + self.h_size, start_x:start_x + self.w_size]
                # Guard against size mismatch (rounding error)
        ...
        clip = np.stack(processed, axis=0).transpose(3, 0, 1, 2)  # (C, T, H, W)
        return torch.from_numpy(clip).float()

    def _load_video_multi_clip(self, video_path):
        try:
            vr = decord.VideoReader(video_path, num_threads=1)
            total_frames = len(vr)
        except Exception:
        ...
        for c in range(self.num_clips):
            indices = self._get_clip_indices(
                total_frames if total_frames > 0 else self.clip_length * self.temporal_stride,
                clip_offset=c,
                num_offsets=self.num_clips
            )
            if total_frames == 0:
                clips.append(torch.zeros(3, self.clip_length, self.h_size, self.w_size))
                continue
            try:
                vr2 = decord.VideoReader(video_path, num_threads=1)
                indices_clipped = [min(i, len(vr2) - 1) for i in indices]
                frames = vr2.get_batch(indices_clipped).numpy().astype(np.uint8)
                clips.append(self._process_frames(frames))
            except Exception:
                clips.append(torch.zeros(3, self.clip_length, self.h_size, self.w_size))
...
# ===========================
def evaluate(model, test_loader, device, num_clips=3, use_tta=True, detailed=False):
    """Unified evaluation function (multi-clip + optional flip TTA)"""
    model.eval()
    all_preds = []
    all_labels = []
    all_probs = []
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Evaluating"):
            ...
            # videos: (B, num_clips, C, T, H, W)
            B, NC, C, T, H, W = videos.shape

            # Process clips sequentially to save memory (batch*NC simultaneous token_attn causes OOM)
            clip_logits_sum = None
            head_logits_sum = defaultdict(lambda: None)

            for ci in range(NC):
                clip = videos[:, ci].to(device)  # (B, C, T, H, W)

                if detailed:
                    out = model(clip, return_all=True)
                    lg = out['final']
                    if use_tta:
                        out_f = model(torch.flip(clip, dims=[4]), return_all=True)
                        lg = (lg + out_f['final']) * 0.5
                    clip_logits_sum = lg if clip_logits_sum is None else clip_logits_sum + lg
                    for key in ['backbone', 'stca', 'mgu', 'person', 'fused']:
                        if key in out:
                            hl = out[key]
                            if use_tta and key in out_f:
                                hl = (hl + out_f[key]) * 0.5
                            head_logits_sum[key] = (
                                hl if head_logits_sum[key] is None
                                else head_logits_sum[key] + hl
                            )
                else:
                    lg = model(clip)
                    if use_tta:
                        lg = (lg + model(torch.flip(clip, dims=[4]))) * 0.5
                    clip_logits_sum = lg if clip_logits_sum is None else clip_logits_sum + lg

            logits = clip_logits_sum / NC

            if detailed:
                for key, hl_sum in head_logits_sum.items():
                    head_avg = hl_sum / NC
                    _, head_pred = torch.max(head_avg, 1)
    ...
    top5_acc = top_k_accuracy_score(
        all_labels, all_probs, k=5, labels=range(all_probs.shape[1])
    ) * 100
    head_accuracies = (
        {k: 100 * v / head_total for k, v in head_correct.items()}
...
# -----------------------------------------------------------------
# 07. F1-Score per Class (color-banded bar chart + mean line)
# -----------------------------------------------------------------
def plot_07_f1score_per_class(all_labels, all_preds, num_classes, save_path):
    ...
    ax.set_title('F1-Score per Class (Kinetics-400)', fontsize=18, fontweight='bold')
...
def plot_08_class_performance_detailed(all_labels, all_preds, idx_to_class, num_classes, save_path):
    ...
    for bar, val in zip(bars, bot10_f1):
        ax.text(bar.get_width() + 0.005, bar.get_y() + bar.get_height() / 2,
                f'{val:.3f}', va='center', fontsize=10, fontweight='bold')
    ax.set_yticks(range(10))
    ax.set_yticklabels(bot10_names, fontsize=9)
...
def plot_09_sample_distribution(all_labels, num_classes, save_path):
    ...
    ax.hist(class_counts, bins=15, color='#aed6f1', edgecolor='#3498db', linewidth=1.2)
    ax.axvline(mean_count, color='red', linestyle='--', linewidth=2.5,
    ...
    ax.set_title('Distribution of Samples per Class (Kinetics-400)', fontsize=20, fontweight='bold')
    ax.legend(fontsize=15)
...
def plot_10_confidence_analysis(all_labels, all_preds, all_probs, idx_to_class, num_classes, save_path):
    ...
    ax.bar(bin_centers, bin_accs, width=bar_width, color='#9b59b6',
...
def save_figure_data_csv(all_labels, all_preds, all_probs, idx_to_class, num_classes, output_dir):
    """Saves all data required for generating figures 06-10 as CSV files."""
...
def print_per_class_accuracy(all_labels, all_preds, idx_to_class):
    ...
    print(f"{'='*70}")
    print(f"{'No.':<5} {'Class':<35} {'Correct/Total':<15} {'Accuracy':>10}")
    print(f"{'-'*70}")
    ...
    for i in range(num_classes):
        ...
        print(f"{i+1:<5} {idx_to_class.get(i, f'class_{i}'):<35} "
              f"{correct:>4}/{total:<8} {acc:>8.2f}%")

    print(f"{'-'*70}")
    print(f"{'Mean':<40} {'':<15} {np.mean(accuracies):>8.2f}%")
    print(f"{'='*70}")
    return accuracies
...
# ===========================
def main():
    parser = argparse.ArgumentParser(description='STCA-ViT Test for Kinetics-400')
    ...
    parser.add_argument('--test_root', type=str,
                        default='/home/user/data/kinetics-dataset/k400/test',
                        help='Kinetics-400 test video directory')
    parser.add_argument('--train_csv', type=str,
                        default='/home/user/data/kinetics-dataset/k400/annotations/train.csv',
                        help='train.csv (used to extract class list)')
    parser.add_argument('--test_csv', type=str,
                        default='/home/user/data/kinetics-dataset/k400/annotations/test.csv',
                        help='test.csv annotation file')
    ...
    parser.add_argument('--num_clips', type=int, default=3,
                        help='Number of evaluation clips per video (default: 3)')
    parser.add_argument('--batch_size', type=int, default=None)
    parser.add_argument('--num_workers', type=int, default=8)
    parser.add_argument('--no_tta', action='store_true', default=False,
                        help='Disable flip TTA')
    parser.add_argument('--detailed', action='store_true', default=False,
                        help='Print per-head accuracy')

    # Output
    parser.add_argument('--output_dir', type=str, default='test_results_kinetics')
    ...
    # Auto batch size
    # token_attn seq_len=6272 (8x28x28) has very high memory usage, so use conservative settings.
    # Actual GPU input: batch_size x num_clips clips processed simultaneously.
    if args.batch_size is None:
        if torch.cuda.is_available():
            ...
            if vram < 24:
                ...
                args.batch_size = 12  # H100 80GB: batch x num_clips = 2 x 3 = 6 clips
        else:
            args.batch_size = 1
    ...
    if 'val_accuracy' in checkpoint:
        print(f"   Last Val Acc: {checkpoint['val_accuracy']:.2f}%")
    ...
    # Remove total_ops / total_params keys (thop/ptflops artifacts)
    ...
    detected_embed_dim = args.module_embed_dim
    ...
    # Check missing/unexpected keys when loading with strict=False
    ...
    # ========================
    print(f"\n{'='*60}")
    print(f"Loading Kinetics-400 annotations...")
    print(f"   Train CSV: {args.train_csv}")
    print(f"   Test CSV:  {args.test_csv}")
    print(f"   Test Root: {args.test_root}")
    print(f"{'='*60}")

    # Build class list from train.csv (same order as training)
    ...
    for _, row in test_df.iterrows():
        ...
        video_path = os.path.join(args.test_root, video_name)
        label_str = row.get('label', None)
        if label_str is None or label_str not in class_to_idx:
            skipped += 1
            continue
        if os.path.exists(video_path):
            test_video_paths.append(video_path)
            test_labels.append(class_to_idx[label_str])
        else:
            skipped += 1

    print(f"Test videos found: {len(test_video_paths)} / {len(test_df)} "
          f"(skipped: {skipped})")

    if len(test_video_paths) == 0:
        print("ERROR: No test videos found. Check --test_root and --test_csv paths.")
        return

    test_dataset = Kinetics400TestDataset(
    ...
    mode = f"{args.num_clips}-clip" + (" + Flip TTA" if use_tta else "")
    print(f"\nEvaluation mode: {mode}")
    print(f"Batch size: {args.batch_size}, num_clips: {args.num_clips}\n")
    ...
    # ========================
    print(f"\n{'='*60}")
    print(f"{'TEST RESULTS (Kinetics-400)':^60}")
    print(f"{'='*60}")
    ...
    class_accuracies = print_per_class_accuracy(
    ...
    plot_06_confusion_matrix(
        results['all_labels'], results['all_preds'], idx_to_class,
        os.path.join(args.output_dir, '06_kinetics_confusion.png')
    )

    # 07. F1-Score per Class
    plot_07_f1score_per_class(
        results['all_labels'], results['all_preds'], num_classes,
        os.path.join(args.output_dir, '07_kinetics_f1score.png')
    )
    ...
    plot_08_class_performance_detailed(
        ...
        os.path.join(args.output_dir, '08_kinetics_class_performance_detailed.png')
    )

    # 09. Distribution of Samples per Class
    plot_09_sample_distribution(
        results['all_labels'], num_classes,
        os.path.join(args.output_dir, '09_kinetics_distribution_of_samples_per_class.png')
    )
    ...
    ece = plot_10_confidence_analysis(
        ...
        os.path.join(args.output_dir, '10_kinetics_prediction_confidence_analysis.png')
    )
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write(f"STCA-ViT Test Results - Kinetics-400\n")
        f.write(f"{'='*60}\n")
        ...
        for i in range(num_classes):
            f.write(f"   {idx_to_class[i]:<40} {class_accuracies[i]:.2f}%\n")
